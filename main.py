...
Examples:
  ...
  python main.py --limit 100 --skip-transformation
    ...
    try:
        ...
        # Check error rate and fail if threshold exceeded
        error_rate = result.get('error_rate', 0.0)
        if error_rate > 0.5:
            logger.error("\n" + "=" * 80)
            logger.error("PIPELINE FAILED")
            logger.error("=" * 80)
            logger.error(f"❌ Pipeline failed: {error_rate*100:.1f}% error rate exceeds 50% threshold")
            logger.error(f"   Error count: {result.get('error_count', 0)}")
            logger.error(f"   Total restaurants: {len(result.get('results', [])) + result.get('error_count', 0)}")
            logger.error("=" * 80)
            sys.exit(1)
        ...
        if result.get('success'):
            ...
            if result.get('error_count', 0) > 0:
                logger.info(f"⚠️  Errors encountered: {result['error_count']} ({error_rate*100:.1f}% error rate)")
